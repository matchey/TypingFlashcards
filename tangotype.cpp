...
namespace tango_type{
	...
	int Duo::input(string filename)/*{{{*/
	{
		int tourokusu=0;
		string tmp[10000][2];

		ifstream f(filename);
		// ifstream f("/Users/matchey/Programs/Asobi/asobi/src/duotan.txt");
		if (f.fail()){
	...
	string chssction()/*{{{*/
	{
		string section;
		int ss;
		string fname = "/Users/matchey/Programs/cpp/Asobi/Target/words/";
		cout<<"学習するセクションを入力(1~19):";
		cin>>section;
		ss = atoi(section.c_str());
		if(ss>0&&ss<19){
			fname += to_string(ss*100-99)+"_"+to_string(ss*100) +".txt";
		}else{
			fname = "/Users/matchey/Programs/cpp/Asobi/Target/words/all.txt";
		}
		return fname;
	}/*}}}*/
